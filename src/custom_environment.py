...
# Max Time of an iteration, defined by time module (seconds)
MAX_TIME = 5
...
class CustomEnv(gym.Env):
    """
    Class representing custom Pygame environment that adheres to OpenAI gym framework for Reinforcement Learning

    Parameters
    ----------
    gym : OpenAI gym class
        inherits from gym class
    """
    def __init__(self,env_config={}):
        ...
        """
        # --- Defining action space and observation space for RL agent. Required by gym. ---
        ...
        # Set ball's position to default starting coordinates of ball
    ...
    def reset(self):
        '''
        Method resets the Pygame environment to its initial state. Required by gym. 
        
            Sets ball coordinates to start, velocities to 0, resets flags and display variables, resets net distance to basket metric, re-records start time
    ...
    def step(self, action=np.zeros((2),dtype=np.float)):
        """
        One step of the Pygame environment, where things happen. Required by gym.
        ...
        action : array, optional
            array of actions chosen by agent interacting with environment, by default np.zeros((2),dtype=np.float)

        Returns
        -------
        tuple
            (array of floats) observation data, (float) reward metric, (bool) doneness flag, (dict) metadata
        """ 
    ...
    def render(self):
        """
        Render the Pygame objects, display info text
        """
    ...
    def pressed_to_action(self, inputnumber):
        """
        User input function for testing throw action

        Parameters
        ----------
        inputnumber : int
            number dictates what action is taken

        Returns
        -------
        array (float, float, bool)
            array of action choices to be passed to step
        """
        velocity_choice = 0
        angle_choice = 0
        throw_variable = 0
    ...
    def get_throw(self):
        '''
        Getter for angle and velocity of throw
        '''
        return self.chosen_angle, self.chosen_velocity
    
    def get_net_distance(self):
        '''
        Getter for net distance metric
        '''
        return self.net_distance_to_basket
    
    def get_outcome(self):
        '''
        Getter for whether a basket was made, numerical output
        1 = bucket made
        0 = none made
        '''
        if self.buckets_flag: return 1
        return 0


# Let user play game at 30FPS
# Press "w" to throw ball 
